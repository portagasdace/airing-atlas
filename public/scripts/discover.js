(() => {
  ...
  const cardHtml = (anime) => {
    const title = titleFor(anime);
    const cover = anime.coverImage?.large || anime.coverImage?.extraLarge || "/og-default.svg";
    return `
      <article class="result-card">
        <a class="result-poster" href="/anime/${escapeHtml(anime.slug)}/" aria-label="Open ${escapeHtml(title)}">
          <img src="${escapeHtml(cover)}" alt="" loading="lazy" referrerpolicy="no-referrer" />
        </a>
        <div>
          <p class="eyebrow">${escapeHtml(seasonLabel(anime))}</p>
          <h3><a href="/anime/${escapeHtml(anime.slug)}/">${escapeHtml(title)}</a></h3>
